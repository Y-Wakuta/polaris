...
- Added MySQL as a supported database for the relational JDBC persistence backend. Set `polaris.persistence.relational.jdbc.database-type` to `mysql` to get started. The MySQL JDBC driver is GPL-licensed and is not distributed with Polaris; build a custom `polaris-server-mysql` runner with `-PincludeMysqlDriver=true` to include the driver locally. See `runtime/server-mysql/README.md`.
